...
storage_client = create_storage_client(supabase_url, supabase_key)
BUCKET_NAME = "uploads"
...
def download_file_from_storage(file_path, local_path):
    ...
    try:
        # Get file content
        response = storage_client.from_(BUCKET_NAME).download(file_path)
...
def get_file_url(file_path, expires_in=3600):
    ...
    try:
        signed_url = storage_client.from_(BUCKET_NAME).create_signed_url(
            file_path,
            expires_in
        )
