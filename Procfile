...
worker: cd backend && rq worker --with-scheduler --url ${REDIS_URL:-redis://${REDISUSER:-default}:${REDISPASSWORD}@${REDISHOST:-localhost}:${REDISPORT:-6379}} --name worker-${RAILWAY_SERVICE_ID:-local} --verbose --worker-ttl 3600 --results-ttl 3600 --burst false default
