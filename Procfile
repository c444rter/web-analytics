...
worker: cd backend && rq worker --with-scheduler --host ${REDISHOST} --port ${REDISPORT} --password ${REDISPASSWORD} --name worker-${RAILWAY_SERVICE_ID:-local} --verbose --worker-ttl 3600 --results-ttl 3600 --max-jobs 0 default
